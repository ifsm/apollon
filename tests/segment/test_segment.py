# pylint: disable = C0114, C0115, C0116
"""test_segment.py
"""
import unittest

from hypothesis import given
from hypothesis.strategies import integers, data, composite, SearchStrategy, DrawFn, DataObject
from hypothesis.extra.numpy import array_shapes
import numpy as np

from apollon.audio import AudioFile
from apollon.segment import Segments, ArraySegmentation
from apollon.segment.models import SegmentationParams


MAX_NSEGS = 345    # cannot pass instance attribute to method decorator


def _valid_nfr() -> SearchStrategy[int]:
    return integers(min_value=2,
                    max_value=20000)

def _valid_nps(n_frames: int) -> SearchStrategy[int]:
    return integers(min_value=2,
                    max_value=n_frames)

def _valid_nol(n_perseg: int) -> SearchStrategy[int]:
    return integers(min_value=1,
                    max_value=n_perseg-1)

@composite
def valid_nfr_nps_nol(draw: DrawFn) -> tuple[int, int, int]:
    nfr = draw(_valid_nfr())
    nps = draw(_valid_nps(nfr))
    nol = draw(_valid_nol(nps))
    return nfr, nps, nol


class TestSegments(unittest.TestCase):
    def setUp(self) -> None:
        seg_params = SegmentationParams(n_perseg=1024, n_overlap=512, extend=True, pad=True)
        self.segs = Segments(seg_params, np.empty((1024, 30)))

    @given(valid_nfr_nps_nol())
    def test_bounds_idx0_negative(self, shape: tuple[int, int, int]) -> None:
        nfr, nps, nol = shape
        params = SegmentationParams(n_perseg=nps, n_overlap=nol, extend=True, pad=True)
        segs = Segments(params, np.empty((nps, nfr)))
        start, _ = segs.bounds(0)
        self.assertLess(start, 0)
        self.assertEqual(start, -(nps//2))    # parens because of floor div

    @given(integers(max_value=-1))
    def test_center_idx_gteq_zero(self, seg_idx: int) -> None:
        with self.assertRaises(IndexError):
            self.segs.bounds(seg_idx)

    @given(data())
    def test_center_idx_lt_nsegs(self, data: DataObject) -> None:
        seg_idx = data.draw(integers(min_value=self.segs.n_segs))
        with self.assertRaises(IndexError):
            self.segs.bounds(seg_idx)

    @given(integers(max_value=-1))
    def test_bounds_idx_gteq_zero(self, seg_idx: int) -> None:
        with self.assertRaises(IndexError):
            self.segs.bounds(seg_idx)

    @given(data())
    def test_bounds_idx_lt_nsegs(self, data: DataObject) -> None:
        seg_idx = data.draw(integers(min_value=self.segs.n_segs))
        with self.assertRaises(IndexError):
            self.segs.bounds(seg_idx)

    def test_data(self) -> None:
        seg_data = self.segs.data
        self.assertIsInstance(seg_data, np.ndarray)

    def test_extend_true(self) -> None:
        self.assertEqual(self.segs._offset, 0)

    def test_extend_false(self) -> None:
        n_perseg = 1024
        n_overlap = 512
        n_segs = 30
        seg_params = SegmentationParams(n_perseg=n_perseg, n_overlap=n_overlap,
                                        extend=False, pad=True)
        segs = Segments(seg_params, np.empty((n_perseg, n_segs)))
        self.assertEqual(segs._offset, n_perseg//2)


class TestSegmentation(unittest.TestCase):
    """
       nps -> n_perseg
       nol -> n_overlap
       gt  -> greater than
       lt  -> less than
       eq  -> eqaul to
    """
    def setUp(self) -> None:
        self.snd = AudioFile('audio/beat_5ch.wav')

    @given(integers(max_value=0))
    def test_nps_gt_zero(self, n_perseg: int) -> None:
        with self.assertRaises(ValueError):
            ArraySegmentation(n_perseg, 1)

    @given(data())
    def test_nps_lteq_nframes(self, data: DataObject) -> None:
        n_perseg = data.draw(integers(min_value=self.snd.n_frames+1))
        cutter = ArraySegmentation(n_perseg, 1)
        with self.assertRaises(ValueError):
            cutter.transform(self.snd.data.squeeze())

    @given(data())
    def test_nol_gt_zero(self, data: DataObject) -> None:
        nov_min_max = (None, 0)
        n_perseg = data.draw(self._valid_nps())
        n_overlap = data.draw(integers(*nov_min_max))
        with self.assertRaises(ValueError):
            ArraySegmentation(n_perseg, n_overlap)

    @given(data())
    def test_nol_lt_nps(self, data: DataObject) -> None:
        n_perseg = data.draw(self._valid_nps())
        n_overlap = data.draw(integers(min_value=n_perseg))
        with self.assertRaises(ValueError):
            ArraySegmentation(n_perseg, n_overlap)

    @given(data())
    def test_inp_lt_three(self, data: DataObject) -> None:
        n_perseg = data.draw(self._valid_nps())
        n_overlap = data.draw(_valid_nol(n_perseg))
        inp_shape = data.draw(array_shapes(min_dims=3))
        inp = np.empty(inp_shape)
        cutter = ArraySegmentation(n_perseg, n_overlap)
        with self.assertRaises(ValueError):
            cutter.transform(inp)

    @given(integers(min_value=2, max_value=1000))
    def test_inp2d_only_one_col(self, n_cols: int) -> None:
        n_frames = 1000
        n_perseg = 50
        n_overlap = 10
        inp = np.empty((n_frames, n_cols))
        cutter = ArraySegmentation(n_perseg, n_overlap)
        with self.assertRaises(ValueError):
            cutter.transform(inp)

    def _valid_nps(self) -> SearchStrategy[int]:
        return _valid_nps(self.snd.n_frames)


if __name__ == '__main__':
    unittest.main()
