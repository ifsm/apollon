...
def log_mel_energies(power: FloatArray, fbank: FloatArray,
    ...
    band of the whole spectrogram**. Bands that far down carry numerical
    residue rather than signal, and the logarithm magnifies it: two empty
    bands at ``1e-12`` and ``1e-14`` read 20 dB apart. The DCT would spread
    that fiction over every coefficient.

    Note what the reference makes of this transform: the floor depends on the
    largest value in ``power``, so the result for one segment depends on every
    other segment passed in the same call. Cutting a signal into chunks, or
    adding a single loud event, moves the floor and hence the coefficients of
    unrelated segments. This matches ``librosa.power_to_db``, whose default
    ``axes="auto"`` reduces over frequency and time alike.
    ...
    Args:
        ...
        top_db:  Width of the retained dynamic range in dB, measured down
                 from the loudest band. If ``None``, do not bound it
    ...
    if top_db is not None:
        energies = np.maximum(energies, energies.max() - top_db)
    ...
    def __init__(self, stft: StftParams, fb: TriangFilterSpec,
        ...
        Args:
            ...
            top_db:       Width of the retained dynamic range in dB, measured
                          down from the loudest band of the spectrogram, see
                          :func:`log_mel_energies`. Note that this couples the
                          segments of one transform to each other
