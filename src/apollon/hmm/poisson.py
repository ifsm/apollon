...
class PoissonHmm:
    ...
    __slots__ = ['hyper_params', 'init_params', 'params', 'decoding',
                 'quality', 'verbose', 'version', 'training_date', 'success']
    ...
    def __init__(self, train_data: Array, m_states: int,
        ...
        self.hyper_params = _HyperParams(m_states, init_lambda, init_gamma,
                                         init_delta, g_dirichlet, d_dirichlet,
                                         fill_diag)
    ...
    def fit(self, train_data: Array) -> None:
        ...
        res = _ca.hmm_poisson_fit_em(train_data, self.hyper_params.m_states,
                                     *self.init_params.__dict__.values(), 1000,
                                     1e-5)
...
class _HyperParams:
    ...
    """Check and save model hyper parameters. Meant for compositional and
    internal only use.  """
    ...
    def __init__(self, m_states: int, init_lambda: ArrayOrStr,
        ...
        self.gamma_dp = _tools.assert_and_pass(self._assert_dirichlet_param,
                                               gamma_dp)
        self.delta_dp = _tools.assert_and_pass(self._assert_dirichlet_param,
                                               delta_dp)
        self.fill_diag = _tools.assert_and_pass(ahu.assert_st_val, fill_diag)

        self.init_lambda_meth = self._assert_lambda(init_lambda)
        self.init_gamma_meth = self._assert_gamma(init_gamma, gamma_dp,
                                                  fill_diag)
    ...
    def _assert_lambda(self, lambda_: ArrayOrStr) -> Array:
        ...
        if isinstance(lambda_, str):
            if lambda_ not in ahu.StateDependentMeansInitializer.methods:
                msg = f'Unrecognized initialization method `{lambda_}`.'
                raise ValueError()

        elif isinstance(lambda_, _np.ndarray):
            _tools.assert_array(lambda_, 1, self.m_states, 0,
                                name='init_lambda')

        else:
            msg = ('Unrecognized type of param ``init_lambda``. Expected '
                   '``str`` or ``numpy.ndarray``, got {type(lambda_)}.')
            raise TypeError(msg)
        return lambda_
...
class _InitParams:
    ...
    def _init_gamma(hy_params: _HyperParams) -> Array:
        ...
        if hy_params.init_gamma_meth == 'dirichlet':
            return ahu.TpmInitializer.dirichlet(hy_params.m_states,
                                                hy_params.gamma_dp)
        ...
        if hy_params.init_gamma_meth == 'uniform':
            return ahu.TpmInitializer.uniform(hy_params.m_states,
                                              hy_params.fill_diag)
